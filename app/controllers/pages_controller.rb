class PagesController < ApplicationController
  def home
    @rooms = Room.order('RANDOM()')
  end

  def search
    session[:aiirbnb_search] = params[:search] if params[:search].present? and params[:search].strip != ''
    arrResult = []
    @rooms_address = if session[:aiirbnb_search] && session[:aiirbnb_search] != ''
                       Room.where(active: true).near(session[:aiirbnb_search], 5, order: 'distance')
                     else
                       Room.where(active: true).all
                     end
    @search = @rooms_address.ransack(params[:q])
    @rooms = @search.result
    @arrRooms = @rooms.to_a
    if params[:start_date] && params[:end_date] && !params[:start_date].empty? & !params[:end_date].empty?
      start_date = Date.parse(params[:start_date])
      end_date = Date.parse(params[:end_date])
      @rooms.each do |room|
        not_available = room.reservations.where("(? <= start_date AND start_date <= ?) OR
            (? <= end_date AND end_date <= ?) OR (start_date < ? AND ? < end_date)", start_date, end_date,
                                                start_date, end_date, start_date, end_date).limit(1)
        @arrRooms.delete(room) if not_available.length > 0
      end
    end
  end

end